...
the minimum number of results rendered per idle slice.  After rendering
that minimum, Johnson keeps draining fast results until
~johnson-render-batch-time-budget~ expires or Emacs has pending input.
The default time budget is ~0.05~ seconds.

#+begin_src emacs-lisp
(setopt johnson-render-batch-time-budget 0.1)
#+end_src

The user option ~johnson-render-idle-delay~ (default ~0.0~) sets
the idle time that must elapse before the next slice fires.  Raise
this value only if you want an additional pause between slices.
