...
In version 0.3, the package supports the DSL (ABBYY Lingvo),
StarDict, and MDict formats, including dictzip-compressed files
(=.dsl.dz= and =.dict.dz=) and encrypted MDict files.  Future
versions will add support for BGL (Babylon) format.
...
module—=johnson-dsl.el= and =johnson-stardict.el=—that registers itself
with the core via the format registry.
...
Here is a sample configuration using =elpaca= and =use-package=:
...
** Version 0.3 (current) --- MDict and DSL abbreviations
:PROPERTIES:
