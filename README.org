...
- *History* / *Sections* / *Modes* --- state and settings.

Toggleable options are placed within the group they relate to rather
than in a separate section.  For example, source and target language
(=-s=, =-t=) and full-text search (=-f=) appear under Search; image
display (=-i=) appears under Copy & media; and global behavior
toggles (=-e= eldoc, =-n= scan, =-r= ref scope) appear under Modes.
