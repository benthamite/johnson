...
dependencies, relying on Emacs 29.1's built-in sqlite support for
...
The development repository is on [[https://github.com/benthamite/johnson][GitHub]].
...
Discovery runs once per session, even when it finds no dictionaries;
run ~johnson-close-caches~ or ~johnson-index~ to scan the directories
again after adding files.
...
The user option ~johnson-default-search-scope~ sets the search scope in
effect until you choose one interactively.  When set to ~all~ (the
default), lookups search across all discovered dictionaries, ignoring
group and language filters.  When set to ~group~, lookups search only
within the active dictionary group: the first entry of
~johnson-dictionary-groups~ when that option is set and no group has
been chosen yet, otherwise the group or language pair chosen with
~johnson-select-group~.  Interactive choices are recorded for the
session and never modify this option.
...
~johnson-select-group~ ([[#h:managing-groups][Managing groups and scope]]), or
...
mapping keys to integer priorities.  Each key is matched first against
the dictionary's metadata name (from the format-specific header), and
then against each ancestor folder name of the dictionary file up to
the configured ~johnson-dictionary-directories~ root, innermost first.
Lower numbers display first.  The default priority is 0, so
dictionaries without an explicit priority entry are all treated
equally and appear in discovery order.

The folder-name fallback means a stable, descriptive directory name
(e.g., =Oxford English Dictionary 2 (2009)=) is usually a more
convenient key than the metadata name, which can vary by format and
release.  Ancestor lookup also handles dictionaries stored one or more
directories below their human-named folder (e.g., a =.dsl= file under
a =dic/= subdirectory).
...
~natnum~).  Setting it to ~0~ disables history.  The persistent
history log ([[#h:browsing-history][Browsing history]]) keeps ten times this many entries.
...
The persistent history log and the bookmarks file are written
atomically: the data goes to a temporary file in the same directory
that is then renamed over the target, so an interrupted write never
truncates the file.  Before each write, entries added by another Emacs
session since this one last read the file are merged in, while entries
...
Each bookmark records the headword, dictionary name, and date.  The file
is written atomically and merged with concurrent sessions' additions
([[#h:lookup-history][Lookup history]]).
...
expensive prefix queries against the full index.  Candidates are
matched case- and accent-insensitively, so typing =cafe= offers =Café=
and =café au lait=.  Forms of one headword that differ only in case or
accents, as when one dictionary stores =LEXICOGRAPHER= and another
=lexicographer=, appear as a single candidate: the form most
dictionaries use, or the least capitalized one when they tie.
...
#+vindex: johnson-render-scheduling-delay
#+vindex: johnson-render-idle-delay
#+vindex: johnson-render-batch-time-budget
Johnson displays the results buffer and its known table of contents
before reading dictionary entries.  A persistent child Emacs retrieves
entries and reuses backend caches across lookups.  The interactive Emacs
inserts ready entries in bounded ordinary-timer slices and checks for
pending input before every unit.

The table of contents lists the local dictionaries known to match when
the lookup starts.  Remote DICT sections cannot be known in advance,
so their items are appended to the table of contents progressively, as
each server reports a match ([[#h:dict-protocol][DICT protocol]]).

The user option ~johnson-render-batch-size~ (default ~1~) is the floor
of each insertion slice: the minimum number of queued render units
(section headers, streamed entries, and section closings) a slice
inserts before its time budget applies.  After inserting that minimum,
Johnson keeps draining ready units until
~johnson-render-batch-time-budget~ (default ~0.05~ seconds) expires or
user input arrives.  Raise the time budget to make all results
available sooner, at the cost of longer pauses when individual entries
are slow to render:

#+begin_src emacs-lisp
(setopt johnson-render-batch-time-budget 0.1)
#+end_src

The user option ~johnson-render-scheduling-delay~ (default ~0.01~)
sets the delay, in seconds, before the next insertion slice is
scheduled.  The slices run on ordinary timers, not idle timers, so
results keep streaming in while you type; each slice checks for
pending input first and yields immediately when there is any.  The
option's former name, ~johnson-render-idle-delay~, is kept as an
obsolete alias.

When the retrieval child fails, the failure is explicit: Johnson never
falls back to retrieving entries in the interactive Emacs silently.
The loading line is replaced with one of the following messages, each
naming the command that opens the diagnostics buffer holding the
details:

#+begin_example
[Johnson retrieval worker exited with status STATUS; run M-x johnson-worker-show-diagnostics]
[Johnson retrieval protocol failed: MESSAGE; run M-x johnson-worker-show-diagnostics]
[Johnson retrieval worker failed to start: MESSAGE; run M-x johnson-worker-show-diagnostics]
#+end_example

An interrupt received while the worker is idle waiting for its next
command is recorded in the diagnostics buffer and does not terminate
the worker.  Interrupts during configuration or retrieval retain the
failure behavior above because partially completed work cannot be
resumed safely.

The worker also disables ~debug-on-event~, whose default ~sigusr2~
value would otherwise arm the batch debugger: a =SIGUSR2= broadcast by
process name at the interactive Emacs (for example =pkill -SIGUSR2
Emacs=) also reaches the worker child, and an armed debugger would
exit it with status 255 at its next activity.

#+findex: johnson-worker-show-diagnostics
The command ~johnson-worker-show-diagnostics~ pops to that diagnostics
buffer, whose space-prefixed name keeps it out of ordinary buffer
switching.

An error in a single dictionary does not abort the lookup: the failing
dictionary renders as an error section and the remaining dictionaries
continue streaming.

The retrieval child's lifetime is decoupled from the results buffer;
see [[#h:cache-management][Cache management]] for when the worker is
...
The retrieval worker is a persistent child Emacs that keeps backend
caches warm across lookups ([[#h:rendering-performance][Rendering performance]]).
Commands that mutate the caches or indexes stop the worker first, so
no retrieval overlaps the mutation: ~johnson-index~,
~johnson-dict-list-reindex~, and ~johnson-close-caches~ all stop it,
and ~johnson-clear-index~ stops it through ~johnson-close-caches~
([[#h:clearing-the-index][Clearing the index]]).  The next lookup starts a
fresh worker automatically.

Killing the =*johnson*= buffer does not stop the worker: it stays
alive so a later lookup can reuse its caches.  When Emacs exits, a
~kill-emacs-hook~ function shuts the worker down, sending it a
protocol shutdown when it is idle and terminating it immediately when
it is blocked in a retrieval.
...
the dictionary entry at point.  When the same headword is bookmarked in
several dictionaries, the bookmark of the dictionary section at point
is removed; outside every section the headword alone decides.  If the
entry is not bookmarked, the command displays a message indicating so.
...
The menu uses a three-column layout with aligned columns.
Toggleable options are placed within the group they relate to:

- Column 1: *Search* (with source/target language, FTS, and ref scope
  options), *Navigate sections*, *Display sections*.
- Column 2: *History* (with persist option), *Bookmarks*, *Copy*,
  *Media* (with images option).
- Column 3: *Dictionaries* (with directories option), *Index*, *Modes*
  (eldoc, scan).
...
A format may also register worker hooks, which let its retrieval run
inside the persistent retrieval child
([[#h:rendering-performance][Rendering performance]]) while rendering
stays in the interactive Emacs:

- ~:worker-query~ --- the query function the worker runs for formats
  without a file-based index, such as the DICT protocol backend
  ([[#h:dict-protocol][DICT protocol]]); it is called with the
  request's word in place of the indexed ~:retrieve-entry~ path.
- ~:worker-prepare-entry~ --- called in the retrieval worker with the
  dictionary plist, the database match, and the raw entry string;
  returns a serializable packet of the form
  =(:raw RAW :context CONTEXT)=.
- ~:render-entry-with-context~ --- called in the interactive Emacs
  with the packet's RAW and CONTEXT values; it must rely only on
  CONTEXT, never on global state set at retrieval time.
- ~:worker-config~ --- returns extra serializable configuration the
  parent sends to the worker at startup.
- ~:apply-worker-config~ --- called in the worker to apply the
  configuration returned by ~:worker-config~.

Formats that register none of these hooks keep the single-argument
~:render-entry~ path and receive a default packet with a ~nil~
context.

#+vindex: johnson-worker-command-function
Worker-side execution requires the format's library to be loadable by
the retrieval child Emacs.  Johnson's bundled formats are loaded
automatically, because the child loads =johnson= itself at startup.  A
third-party format must extend ~johnson-worker-command-function~ so
that the command it returns also loads the format's library in the
child.  A format the child cannot load does not abort the lookup: each
of its dictionaries renders as an error section naming the format that
is not registered in the worker.
...
#+findex: johnson-html-clean-headword
Before a headword reaches the database, the indexing path also passes it
through ~johnson-html-clean-headword~, which removes HTML markup for
every format: digit-only superscripts, which mark homograph senses as in
=Belén<sup>1</sup>=, are dropped entirely; other tags are stripped with
their text kept, so =H<sub>2</sub>O= indexes as =H2O=; entities are
decoded; whitespace is collapsed and trimmed.  A headword consisting only
of markup is not indexed.  DSL bracket tags are handled by the DSL
backend before this point.
...
headword is stored with surrounding whitespace (including tabs and
no-break spaces) removed, and nothing is inserted when it is blank; it
is automatically normalized via ~johnson-db-normalize~
...
="mtime"=) against the actual file's modification time, and the stored
="index-version"= against the constant ~johnson-db-index-version~,
which is bumped whenever a change alters the entries an index stores.
An index built under another version, or one that sqlite cannot open,
is stale and gets rebuilt.  If the sqlite database file does not exist
at all, the function returns non-nil.  This
...
opens a sqlite database; it checks whether the index file exists,
whether the dictionary file's modification time is newer than the index
file's, and whether the cache-wide marker file =index-version= in
~johnson-cache-directory~ records the running ~johnson-db-index-version~.
That marker is written at the end of every full indexing run, so after
an upgrade that bumps the version every dictionary is reported stale
once and rebuilt through the usual indexing prompt.  This makes the
check suitable for rapid scanning during discovery without the overhead
of opening every database.
...
- =[trn]=, =[!trn]=: translation blocks (blank line separation);
  =[trn]= remains inline after a phrase/example translation arrow
- =[com]=: comments
...
#+findex: johnson-html-decode-entities-string
...
** Version 0.6 --- EPWING improvements and audits
...
** Version 0.7 --- Progressive rendering

- [X] Results load progressively without blocking Emacs: a persistent
  child Emacs retrieves and prepares entries while the interactive
  session inserts them in bounded slices that yield to input.
- [X] Remote dictionary sections appear in the table of contents as
  they report matches.
- [X] Worker failures are reported in the results buffer; the command
  ~johnson-worker-show-diagnostics~ opens the worker's diagnostics.
- [X] Formats can participate in worker-side retrieval through new
  ~johnson-register-format~ hooks.
- [X] ~johnson-render-scheduling-delay~ controls insertion pacing.
- [X] Transient menu reorganized into domain-oriented groups with
  source and target language infixes.
- [X] Minimum Emacs version lowered to 29.1; continuous integration
  added.

** Version 0.8 (current) --- Robustness fixes

A code audit of every backend and of the worker, protocol and user
interface layers.  The fixes below each carry a regression test.


- [X] BGL: dictionary title, languages and character sets are read
  from the real Babylon property layout, and headwords are decoded
  with the dictionary's source charset, so non-Latin and accented
  headwords index and match correctly.
- [X] BGL: a failed decompression no longer leaves an empty cache
  buffer that later attempts mistake for a populated one.
- [X] EPWING: corrupt or revisited index pages no longer abort
  indexing with a ~no-catch~ error, and an entry ending exactly on a
  read boundary is no longer merged with the next entry.
- [X] EPWING: gaiji (book-specific characters) render as 〓 instead of
  corrupting the surrounding text or dropping the headword.
- [X] DSL: headword expansion is bounded during expansion, so a
  headword with many optional or alternative parts cannot hang
  indexing; lines inside multi-line ={{ ... }}= comment blocks are no
  longer indexed as headwords.
- [X] StarDict: a truncated =.syn= file no longer prevents indexing,
  and a short read from a =.dict.dz= file reports a clear error.

- [X] dictzip: a truncated or corrupt =.dict.dz= or =.dsl.dz= file
  reports an error instead of silently yielding a cut-off definition;
  very large files (over about 32,000 chunks) now open, and a
  cut-off header reports a clear error.
- [X] EBZIP and MDict: a forged size or block-count field can no longer
  make Emacs allocate gigabytes of memory; unsupported EBZIP
  compression levels are rejected with a clear message.
- [X] MDict: entries can no longer reference files outside the
  dictionary's own folder.

- [X] Retrieval worker: when the worker crashes, its final output,
  including the error text it printed, now reaches the diagnostics
  buffer instead of being lost.
- [X] DICT protocol: a lookup that times out or is interrupted drops
  its connection, so later lookups can no longer receive the previous
  word's definitions; a server that closes the connection fails the
  lookup at once instead of after the full timeout.
- [X] Index database: a zero-byte or corrupt index file is detected as
  stale and rebuilt automatically instead of breaking that dictionary
  until the file is deleted by hand.

- [X] Eldoc mode shows dictionary definitions; previously the echo
  area was cleared immediately after the definition appeared.
- [X] Headword completion keeps candidates that differ from the input
  only in case or accents, so typing =cafe= offers =Café=.
- [X] Full-text indexing no longer fails for a whole dictionary when an
  entry contains an out-of-range numeric HTML entity.
- [X] The transient menu binds *Reorder dictionaries* to =O=; it
  previously shared =r= with *Forward* and was unreachable.
- [X] =j= (jump to section) and =p= (previous section) work in
  single-dictionary results, whose only header starts at the top of
  the buffer.
- [X] Enabling scan mode while it is already on no longer leaves an
  extra idle timer running.
- [X] Full-text search results keep their query separate from the
  headword state: =g= reruns the search, and bookmarking a search page
  is refused instead of storing a bogus headword.
- [X] Sound links and images in HTML dictionaries can no longer
  reference files outside the dictionary's folder, and companion-zip
  resource names are passed to =unzip= literally.

- [X] DSL: dictionaries saved as UTF-16 without a byte-order mark are
  recognised and indexed correctly instead of being accepted as empty.
- [X] StarDict: a dictionary whose =.ifo= declares the wrong word count
  is indexed instead of rejected, with a message noting the mismatch.
- [X] BGL: a truncated dictionary file produces a warning naming the
  file instead of silently dropping its last entry, while ordinary
  Babylon files, whose checksum is always zero, load without any
  message; lookups no longer copy the whole decompressed dictionary.

- [X] MDict: images and sounds with the same file name in different
  folders of the resource archive, whether written with slashes or
  Windows-style backslashes, no longer overwrite each other in the
  cache, so each entry shows the correct file.

- [X] History and bookmarks are saved atomically and merged across
  concurrent Emacs sessions, so a crash or a second Emacs no longer
  loses entries.
- [X] ~johnson-default-search-scope~ governs the initial scope, and
  ~johnson-select-group~ records its choice for the session instead of
  rewriting the option.
- [X] ~johnson-history-max~ accepts only natural numbers, and ~0~
  disables history instead of keeping one entry.
- [X] Removing a bookmark targets the dictionary section at point when
  the same headword is bookmarked in several dictionaries.
- [X] With no dictionaries found, eldoc and scan mode no longer rescan
  the dictionary directories on every update.
- [X] Indexing progress reports full-text results for dictionaries whose
  names contain spaces.
- [X] Audio and image files extracted from companion =.dsl.files.zip=
  archives no longer overwrite each other when two members share a
  file name in different folders.

- [X] Headwords are stored with surrounding whitespace removed and blank
  headwords dropped, so padded variants such as =Lexicography = no
  longer appear as separate completion candidates.
- [X] Indexes carry a format version; indexes built by an earlier
  version are detected on first use and rebuilt through the usual
  indexing prompt, so fixes that change what gets indexed apply
  without manual reindexing.

- [X] DSL: headword lines that carry formatting tags (part of speech,
  feminine endings, italics) are indexed as the clean headword, so
  entries such as =lexicógrafo= are found instead of being stored as
  fragments of the tags.
- [X] DSL: body text that a converter left without indentation
  (recognisable by margin tags or excessive length) is kept inside its
  entry instead of being indexed as a multi-kilobyte headword;
  indexing reports how many such lines were reclassified.

- [X] Headwords are stored without HTML markup for every format, so
  =Belén<sup>1</sup>= indexes as =Belén= and =H<sub>2</sub>O= as
  =H2O=, and markup-only keys are dropped.

- [X] MDict: =@@@LINK== redirect targets are cleaned the same way as
  indexed headwords, so links to keys that carried markup still resolve.
...
* Contributing
:PROPERTIES:
:CUSTOM_ID: h:contributing
:END:

=johnson= is distributed through [[https://melpa.org/][MELPA]].  Copyright is held
by the author; no copyright assignment is required to contribute.

Bug reports and patches can be sent to the [[https://github.com/benthamite/johnson/issues][issue tracker]].

** AI assistance

=johnson= was written with substantial help from large language models
(Claude and Codex), with the author reviewing all generated code.
Following MELPA's contributor guidelines, every source file carries an
=Assisted-by= line beneath its =Author= line recording this.
Contributors who use such tools for their own patches are asked to
keep that line accurate.
...
Tests that require zlib (dictzip, mdict decompression) are automatically
skipped when Emacs is compiled without zlib support.
