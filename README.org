...
In version 0.2, the package supports the DSL (ABBYY Lingvo) and
StarDict formats, including dictzip-compressed files (=.dsl.dz= and
=.dict.dz=).  Future versions will add support for MDict and BGL
(Babylon) formats.
...
** Version 0.2 (current) --- StarDict and compressed DSL
:PROPERTIES:
